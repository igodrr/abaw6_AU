...
class MyWarmupScheduler(object):
    ...
    def warmup_lr(self, init_lr, batch,  num_batch_warm_up, epoch = None):
        if batch < num_batch_warm_up: #self.num_warmup_epoch:
            for params in self.optimizer.param_groups:
                params['lr'] = (batch+1) * params['initial_lr'] / (num_batch_warm_up)# * self.num_warmup_epoch + 1e-100)
        else:
            for params in self.optimizer.param_groups:
