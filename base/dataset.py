...
dataset_path='/home/data/lrd/zgp/abaw/Affwild2_processed'
...
class GenericDataArranger(object):
    ...
    def generate_partitioned_trial_list(self, window_length, hop_length, fold, windowing=True):
        ...
        partition_range = list(train_validate_range)#np.roll(train_validate_range, fold))
...
class GenericDataset(Dataset):
    def __init__(self, data_list, continuous_label_dim, modality, multiplier, feature_dimension, window_length, mode, mean_std=None,
        ...
        # self.all_examples = {}
        # self.pre_modality = ["AU_continuous_label"]
        # self.after_modality = ["video","mfcc","vggish"]
        # for dl in self.data_list:
        #     path, trial, length, index = dl
        #     examples = {}
        #     for feature in self.pre_modality:
        #         if feature == 'AU_continuous_label':
        #             path = path.replace('/home/data/lrd/zgp/abaw/Affwild2_processed/','/home/data/zhangzr22/abaw/ABAW5/code_AU_EXPR_VA/data/')
        #         examples[feature] = self.get_example(path, length, index, feature)
        #     if examples['AU_continuous_label'].min() < 0 :
        #         examples['AU_continuous_label'] = np.where(examples['AU_continuous_label'] == -1, 0, examples['AU_continuous_label'])
        #     self.all_examples[trial]=examples
    ...
    def get_feature_transform(self, feature):
        if ("logmel" in feature) or ('bert' in feature):
    ...
    def __getitem__(self, index):
        ...
        examples = {}
        # for feature in self.pre_modality:
        #     examples[feature] = self.all_examples[trial][feature]

        for feature in self.modality:
            # if feature == 'video':
            #     path = path.replace('video','video128')
            if feature == 'bert':
                path = path.replace('/home/data/lrd/zgp/abaw/Affwild2_processed/','/home/data/lrd/zgp/abaw/dataset/')
            if feature == 'AU_continuous_label':
                path = path.replace('/home/data/lrd/zgp/abaw/Affwild2_processed/','/home/data/zhangzr22/abaw/ABAW5/code_AU_EXPR_VA/data/')
            examples[feature] = self.get_example(path, length, index, feature)
        # if examples['AU_continuous_label'].min() < 0 :
        #     examples['AU_continuous_label'] = np.where(examples['AU_continuous_label'] == -1, 0, examples['AU_continuous_label'])
        # if 'bert' in self.modality:
        #     print(torch.sum(examples['bert'] != 0))
    ...
    def load_data(self, path, indices, feature):
        ...
        # filename = filename.replace('video','video128')
